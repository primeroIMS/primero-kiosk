...
function AlertDialogContent({
    ...
    return (
        <AlertDialogPortal>
            <AlertDialogOverlay style={overlayStyles} />
            <AlertDialogPrimitive.Popup
                className={cn(
                    `
                      group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full
                      -translate-x-1/2 -translate-y-1/2 gap-6 rounded-4xl bg-background
                      p-6 ring-1 ring-foreground/5 duration-100 outline-none
...
function AlertDialogMedia({ className, ...props }: React.ComponentProps<"div">) {
    return (
        <div
            className={cn(
                `
                  mb-2 inline-flex size-16 items-center justify-center rounded-full
                  bg-muted
