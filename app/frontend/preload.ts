...
async function loadAppFlows() {
    const response = await api.get(ENDPOINTS.appFlows);
    AppFlowStore.setData(response.data.data);
}

async function loadLookups() {
    const response = await api.get(ENDPOINTS.lookups);
    LookupStore.setLookups(response.data.data);
}

async function loadSystemSettings() {
    const response = await api.get(ENDPOINTS.systemSettings);
    SystemSettingsStore.setSettings(response.data.data);
}

async function loadTheme() {
    const response = await api.get(ENDPOINTS.theme);
